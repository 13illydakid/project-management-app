...
function ExplorerColumn({
  ...
  const handleToggle = async (item: ExplorerItem) => {
    ...
    <div className="min-w-[22rem] sm:min-w-[26rem] min-h-[400px] backdrop-blur-xl bg-black/30 border border-white/10 text-gray-100 rounded-2xl shadow p-4 mr-4 flex flex-col gap-2">
      ...
      {type === "preview"
        ...
        : items.map((item) => (
            <div key={item.id}>
              <div
                className={`cursor-pointer px-3 py-2 rounded-lg flex items-center justify-between transition border ${
                  selectedId === item.id
                ...
                onDrop={
                  type === "folder"
                    ? (e) => {
                        ...
                        if (fileId) onSelect({ ...item, dropFileId: fileId });
                      }
                    : undefined
                }
                onDragOver={
              ...
              >
                <span className="flex items-center gap-2 font-semibold break-words">
                  {item.note && (
                    <span
                      className={`${
                        item.type === "folder" ? "bg-orange-500" : "bg-red-500"
...
function PreviewPane({ item }: { item: ExplorerItem }) {
  ...
  if (mime === "application/pdf" || url.match(/\.pdf$/i)) {
    return (
      <object
        ...
        className="w-[420px] h-[360px] border rounded"
      >
        <a
          href={url}
          target="_blank"
  ...
  if (/video\//.test(mime)) {
    return <video src={url} controls className="w-[420px] h-[300px]" />;
  }
  return (
    <div className="p-2">
              ...
              <td className="px-3 py-2">
                <span className="inline-flex items-center gap-2">
                  {it.note && (
                    <span
                      className={`${
                        it.type === "folder" ? "bg-orange-500" : "bg-red-500"
                      } inline-block w-2 h-2 rounded-full`}
                      aria-label="Has note"
                    />
                  )}
    ...
    >
      ...
      >
        <div className="flex items-center gap-4 mb-6">
          ...
          <Link
            href={`/dashboard/${project.id}/new`}
            className="ml-auto bg-white/10 border border-white/20 text-gray-100 px-3 py-1 rounded text-sm hover:bg-white/15"
          >
            + New Task
          </Link>
        </div>
